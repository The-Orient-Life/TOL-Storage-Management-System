...
function NewPurchaseCustomerSearch({ onCustomerSelect }) {
  ...
  const handleSearch = () => {
    const customer = searchCustomer(nic);
    if (customer) {
      ...
      {searchResult === 'not_found' && !showForm && (
        <div className="bg-red-50 border border-red-200 p-4 rounded-lg flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-red-600 text-sm">Please register the customer before proceeding with the purchase.</p>
          </div>
        </div>
      )}
      ...
      {showForm && (
        <div className="bg-white p-6 rounded-lg shadow-md border border-gray-100">
          <div className="flex items-center gap-2 mb-6">
            ...
            <h3 className="text-xl font-semibold">New Customer Registration</h3>
          </div>
          <form className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium mb-1 text-gray-700">NIC</label>
                <input
                  type="text"
